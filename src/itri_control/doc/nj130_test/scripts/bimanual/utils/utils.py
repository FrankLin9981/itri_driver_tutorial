...
def compute_bimanual_goal_configs(robots, obj, q_robots_cur, q_robots_grasp, 
  ...
  def restore():
    ...
    for (robot, q_robot_cur, q_robot_grasp) in zip(robots, q_robots_cur,
                                                   q_robots_grasp):
      robot.Enable(True)
      if reset: robot.SetDOFValues(np.append(q_robot_grasp, q_robot_cur))
  ...
  for (robot, q_robot_cur, q_robot_grasp, index) in \
    ...
    gripper_limit = robot.GetDOFLimits()[1][0:2]
    robot.SetDOFValues(np.append(q_robot_grasp + (gripper_limit - q_robot_grasp) * 0.1,
                                  q_robot_new))
